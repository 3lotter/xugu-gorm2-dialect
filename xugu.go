...
import (
	...
	_ "github.com/3lotter/xugu-driver-go"
...
func (dialector Dialector) getSchemaIntAndUnitType(field *schema.Field) string {
	...
	constraint := func(sqlType string) string {
		...
		if field.AutoIncrement {
			parts := strings.Split(field.StructField.Tag.Get("gorm"), "AUTO_INCREMENT")
			if len(parts) >= 2 {
				values := strings.Split(parts[1], ";")[0]
				sqlType += " identity(" + values[1:] + ",1)"
			} else {
				sqlType += " identity"
			}
		}
